...
- Update notification on local web dashboard was already implemented via
  `/api/admin/update/check` and `/api/admin/update/run` — verified working in
  v1.6.3 Settings → System tab. This release only ensures the TUI experience
  matches the reliability expectations set by the web admin UI.
